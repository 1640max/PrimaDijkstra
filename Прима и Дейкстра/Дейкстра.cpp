...
int main()
{
	...
	for(int i=0;i<N;i++)
		{
			for(int j=0;j<N;j++)
			{
				if(i==j)
				{
					printf("Rasstojanie iz uzla %d v %d = 0",i+1,j+1);
					A[i][j]=0;
					puts("");
				}
				else
				{
				printf("Rasstojanie iz uzla %d v %d = ",i+1,j+1);
				scanf("%d",&A[i][j]);
				}
	...
	printf("Poluchivshajasa matriza:\n");
	for(int i=0;i<N;i++)
		{
			for(int j=0;j<N;j++)
			printf("%d ",A[i][j]);
	...
	printf("Vvedite nachalnauju vershinu - ");
	scanf("%d",&uzel);
   Deikstra(A, uzel-1, N);
   getch();
   return 0;
